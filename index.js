...
reply(mess.wait)
kon = await getBuffer(`https://api-alc.herokuapp.com/api/nsfw/yuri?&apikey=${hardi}`)
...
if (!isNsfw) return reply(`La función nsfw aún no está activa en este grupo\nEscribe ${prefix}nsfw 1`)
reply(mess.wait)
anu = await fetchJson(`https://api-alc.herokuapp.com/api/nsfw/hentai?apikey=ConfuMods`)
buffer = await getBuffer(anu.url)
cnf.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./cnf.jpg')})
break
case  'megumin':
